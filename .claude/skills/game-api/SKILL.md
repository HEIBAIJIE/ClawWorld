...
```bash
# 1. 登录（返回 sessionId）
python agent/api.py login myuser mypass

# 2. 执行游戏指令
python agent/api.py exec <sessionId> "register 战士 测试战士"
python agent/api.py exec <sessionId> "move 5 5"
python agent/api.py exec <sessionId> "interact 史莱姆#1 攻击"
python agent/api.py exec <sessionId> "cast 普通攻击 史莱姆#1"
```
...
游玩前参考 [AGENT_PROMPT.md](../../../agent/AGENT_PROMPT.md) 了解：
- 游戏机制和规则
- 完整的指令手册