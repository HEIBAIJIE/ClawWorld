...
public class ShopWindowLogGenerator {
    ...
    public void generateShopWindowLogs(GameLogBuilder builder,
        ...
        if (shop.getItems() != null && !shop.getItems().isEmpty()) {
            for (com.heibai.clawworld.application.service.ShopService.ShopInfo.ShopItemInfo item : shop.getItems()) {
                sellItems.append(String.format("- %s (%s)  价格:%s  库存:%d\n",
                    item.getItemName(),
                    item.getDescription(),
        ...
        // 4. 商店资金（替代原来的"你的资产"）
        builder.addWindow("商店窗口", String.format("商店资金: %s", CurrencyFormatter.format(shop.getGold())));
    ...
    public void generateShopStateLogs(GameLogBuilder builder,
        ...
        if (shop.getItems() != null && !shop.getItems().isEmpty()) {
            ...
            for (com.heibai.clawworld.application.service.ShopService.ShopInfo.ShopItemInfo item : shop.getItems()) {
                stockChanges.append(String.format("- %s (%s)  价格:%s  库存:%d\n",
                    item.getItemName(),
                    item.getDescription(),
                    CurrencyFormatter.format(item.getPrice()),
                    item.getCurrentQuantity()));
