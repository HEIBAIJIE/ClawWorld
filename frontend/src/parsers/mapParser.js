...
/**
 * 解析实体列表
 * 输入格式: "史莱姆#1 (8,8) [可直接交互] [类型：ENEMY] [交互选项: 查看, 攻击]"
 * 或新格式: "史莱姆#1 Lv.3 (8,8) [可直接交互] [类型：ENEMY] [交互选项: 查看, 攻击]"
...
export function parseEntityList(content) {
  ...
  // 支持两种格式：有等级和无等级
  const entityPatternWithLevel = /^(.+?)\s+Lv\.(\d+)\s+\((\d+),(\d+)\)\s+\[([^\]]+)\]\s+\[类型：(\w+)\]\s+\[交互选项:\s*([^\]]*)\]/
  const entityPatternNoLevel = /^(.+?)\s+\((\d+),(\d+)\)\s+\[([^\]]+)\]\s+\[类型：(\w+)\]\s+\[交互选项:\s*([^\]]*)\]/
  ...
  for (const line of lines) {
    ...
    // 先尝试有等级的格式
    let match = trimmed.match(entityPatternWithLevel)
    if (match) {
      entities.push({
        name: match[1].trim(),
    ...
    // 再尝试无等级的格式
    match = trimmed.match(entityPatternNoLevel)
    if (match) {
      entities.push({
        name: match[1].trim(),
        level: null,
